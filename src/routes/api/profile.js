...
// @desc    Get profile by nick
// @access  Public

router.get('/nickname/:nick', (req, res) => {
  const errors = {};

  Profile.findOne({ nick: req.params.nick })
    .populate('user', ['name', 'avatar'])
    .then(profile => {
      if (!profile) {
        errors.noprofile = 'There is no profile for this user';
...
router.post(
  ...
  (req, res) => {
    ...
    if (req.body.nick) profileFields.nick = req.body.nick;
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      if (profile) {
        ...
        Profile.findOne({ nick: profileFields.nick }).then(profile => {
          if (profile) {
            errors.nick = 'That nick already exists';
